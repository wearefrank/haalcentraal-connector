...
/**
 * Database storage implementation for Ladybug. The configuration of a
 * transaction manager
 * (see {@link Config#ladybugTransactionManager(DataSource)} will disable
 * auto-commit so PostgreSQL will not throw the following
 * exception on insert of a report:
 * org.postgresql.util.PSQLException: Large Objects may not be used in
 * auto-commit mode.
 * It would also be possible to set auto-commit to false on Connection(Pool) or
 * DataSource level but then still a
 * transaction manger needs to be configured for JdbcTemplate to commit changes.
 * Otherwise everything seems to be
 * working fine (logging shows insert query) but no data appears in database and
 * debug tab.
 * 
 * @see OptionalJtaTransactionManager
 * @author Jaco de Groot
 */
// Without proxyTargetClass = true the test webapp will give: Bean named
// 'proofOfMigrationStorage' is expected to be of
// type 'nl.nn.testtool.storage.proofofmigration.ProofOfMigrationStorage' but
// was actually of type 'jdk.proxy3.$Proxy26'
// @EnableTransactionManagement(proxyTargetClass = true)
// REQUIRES_NEW to prevent interference with transactions in the application
// using Ladybug. E.g. when an error occurs in
// a Frank!Framework adapter the insert of the Ladybug report should not be
// rolled back (which happens otherwise because
// transactions are thread bound and Ladybug runs in the same thread). With
// NOT_SUPPORTED PostgreSQL will complain about
// auto-commit.
// Although isolation = Isolation.READ_UNCOMMITTED could be considered for
// performance reasons it will give the
// following error for Oracle in the Frank!Framework test matrix: READ_COMMITTED
// and SERIALIZABLE are the only valid
// transaction levels.
...
public class DatabaseStorage implements Storage {
	...
	protected @Setter @Inject @Resource(name = "metadataNames") List<String> metadataNames; // Used as column names in
																							// this storage
	protected @Setter String storageIdColumn;
	protected @Setter List<String> bigValueColumns; // Columns for which to limit the number of retrieved characters to
													// 100
	...
	// Return defaults in get methods to make is possible for overriding class to
	// return different default values when
	...
	public List<String> getMetadataNames() {
		if (getMetadataExtractor().getExtraMetadataFieldExtractors() != null) {
			...
			for (MetadataFieldExtractor extractor : getMetadataExtractor().getExtraMetadataFieldExtractors()) {
				// ExtraMetadataFieldExtractors can override standard fields (see
				// extraMetadataFieldExtractors in
				// MetadataExtractor)
	...
	public void store(Report report) throws StorageException {
				...
				new PreparedStatementSetter() {
					...
					public void setValues(PreparedStatement ps) throws SQLException {
						...
						for (String column : getMetadataNames()) {
							if (!column.equals(getStorageIdColumn())) {
								if (isInteger(column)) {
									ps.setInt(i, (Integer) metadataExtractor.getMetadata(report, column,
											MetadataExtractor.VALUE_TYPE_OBJECT));
								} else if (isLong(column)) {
									ps.setLong(i, (Long) metadataExtractor.getMetadata(report, column,
											MetadataExtractor.VALUE_TYPE_OBJECT));
								} else if (isTimestamp(column)) {
									ps.setTimestamp(i, new Timestamp((Long) metadataExtractor.getMetadata(report,
											column, MetadataExtractor.VALUE_TYPE_OBJECT)));
								} else {
									ps.setString(i, (String) metadataExtractor.getMetadata(report, column,
											MetadataExtractor.VALUE_TYPE_OBJECT));
								}
								i++;
							}
		...
		if (getMaxReportRetentionDays() > -1) {
			String maxReportsToKeepQuery = "select count(*) from " + getTable()
					+ " where " + getDateColumn() + " >= now() - interval '" + getMaxReportRetentionDays()
					+ " days'";
			...
			System.out.println("Calculated max reports to keep (returned " + maxReportsToKeep + "): " + maxReportsToKeepQuery);
	...
	// StorageException is allowed by Storage.getReport(), hence no need to handle
	// it in the lambda expression that will
	// call this method
	@SneakyThrows
	...
	public List<List<Object>> getMetadata(int maxNumberOfRecords, List<String> metadataNames, List<String> searchValues,
		...
		// Prevent SQL injection (searchValues are passed as parameters to the SQL
		// statement)
	...
	protected void buildMetadataQuery(int maxNumberOfRecords, List<String> metadataNames, List<String> searchValues,
		...
		if (query.charAt(query.length() - 1) == ' ') {
			// Clean up trailing space used by addExpression() to determine to add "where"
			// or "and"
